...
function WorklistNavigationRegular({
  panelDefinition,
  onNewView,
  selectedViewId,
  onPanelTitleChange,
...
function WorklistNavigationReactive({
  panelDefinition,
  onNewView,
  selectedViewId,
  onPanelTitleChange,
  ...
  const router = useRouter();
  const { getSaveState, deleteView } = useReactivePanelStore();
  const [editingPanel, setEditingPanel] = useState(false);
  const [editingViewId, setEditingViewId] = useState<string | null>(null);
  const [hoveredViewId, setHoveredViewId] = useState<string | null>(null);
  const [deletingViewId, setDeletingViewId] = useState<string | null>(null);
...
// Main component that chooses between regular and reactive based on feature flag
export default function WorklistNavigation(props: WorklistNavigationProps) {
  const isReactiveEnabled = isFeatureEnabled('ENABLE_REACTIVE_DATA_STORAGE');

  if (isReactiveEnabled) {
    return <WorklistNavigationReactive {...props} />;
  }

  return <WorklistNavigationRegular {...props} />;
}