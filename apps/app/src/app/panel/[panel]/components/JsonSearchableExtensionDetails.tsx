...
function getExtensionValue(ext: Extension): unknown {
  const valueKey = Object.keys(ext).find(
    (k) => k.startsWith('value') && k !== 'valueSet',
  )
  // @ts-ignore
...
function extensionMatchesSearch(
  ext: Extension,
  searchTerm: string,
...
function filterJsonContent(
  ...
  if (typeof obj === 'string') {
    const matches =
      searchMode === 'keys' ? false : obj.toLowerCase().includes(searchLower)
    if (matches) {
      matchCount = 1
      hasMatches = true
    }
    return {
      matches: hasMatches,
      matchCount,
      filteredData: hasMatches ? obj : null,
    }
  }
  ...
  if (typeof obj === 'number' || typeof obj === 'boolean') {
    const matches =
      searchMode === 'keys'
        ? false
        : String(obj).toLowerCase().includes(searchLower)
    if (matches) {
      matchCount = 1
      hasMatches = true
    }
  ...
  if (Array.isArray(obj)) {
    ...
    for (let i = 0; i < obj.length; i++) {
      const result = filterJsonContent(
        obj[i],
        searchTerm,
        searchMode,
        `${path}[${i}]`,
    ...
    for (const [key, value] of Object.entries(obj)) {
      const keyMatches =
        searchMode === 'values'
          ? false
          : key.toLowerCase().includes(searchLower)
      const valueResult = filterJsonContent(
        value,
        searchTerm,
        searchMode,
        path ? `${path}.${key}` : key,
...
function searchExtensionsWithJsonFilter(
  ...
  for (const ext of extensions) {
    const hasExtensionLevelMatch = extensionMatchesSearch(
      ext,
      searchTerm,
      searchMode,
    )
    const parsedData = parseJsonRecursively(ext)
    const jsonFilterResult = filterJsonContent(
      parsedData,
      searchTerm,
      searchMode,
    )
...
export function JsonSearchableExtensionDetails({
  ...
  const totalMatchCount = searchResults.reduce(
            ...
            <div className="mt-1 text-xs text-gray-500">
              {totalMatchCount > 0 ? (
                <>
                  ...
                  {totalMatchCount === 1 ? 'property' : 'properties'}
                  in {uniqueExtensions}{' '}
                  {uniqueExtensions === 1 ? 'extension' : 'extensions'}
                </>
              ) : (
        ...
        {searchResults.map((result) => {
          ...
          return (
            ...
            >
              <div className="mb-2">
                ...
                {hasActiveSearch && result.matchCount > 0 && (
                  <p className="text-xs text-blue-600 mt-1">
                    {result.matchCount}{' '}
                    {result.matchCount === 1 ? 'property' : 'properties'}{' '}
                    matched
                  </p>
                )}
                {hasActiveSearch &&
                  result.hasExtensionLevelMatch &&
                  result.matchCount === 0 && (
              ...
              {(() => {
                const EnhancedJsonViewerComponent = isFeatureEnabled(
                  'USE_SIMPLIFIED_JSON_VIEWER',
                )
