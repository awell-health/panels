...
export const HighlightedJsonViewer: React.FC<HighlightedJsonProps> = ({
  ...
  const renderValue = (
    ...
    if (typeof value === 'string') {
      ...
      return (
        <span className={`text-green-600 ${highlightClasses}`}>"{value}"</span>
      )
    }
          ...
          {isExpanded && (
            <div className="ml-4">
              {entries.map(([key, val]) => {
                ...
                const { isHighlighted: keyHighlighted } = highlightText(
                  key,
                  searchTerm,
                  searchMode,
                )
                const keyHighlightClasses = getHighlightClasses(
                  'key',
                  keyHighlighted,
                )
                ...
                return (
                  <div key={key} className="flex items-start gap-1">
                    ...
                    <div className="flex-1">{renderValue(val, keyPath)}</div>
                    {keyMatches.length > 0 && (
