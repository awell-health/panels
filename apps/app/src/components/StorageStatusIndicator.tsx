...
export function StorageStatusIndicator({
  ...
  useEffect(() => {
        ...
        if (!cancelled) {
          setStatus({
            ...
            details: {
              ...
              cacheEnabled:
                storageMode === 'api' ? storageConfig.isCacheEnabled : false,
              cacheDuration:
                storageMode === 'api'
                  ? storageConfig.cacheDurationMs
                  : undefined,
              apiConfig: {
    ...
    const interval = setInterval(
      ...
      10 * 60 * 1000,
    ) // Check every 10 minutes to minimize conflicts
              ...
              <div className="space-y-2">
                ...
                {storageConfig.isCacheEnabled && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900">
                      {storageConfig.cacheDurationText}
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Object.entries(storageConfig.presetDurations).map(
                      ([preset, duration]) => (
                        <button
                          key={preset}
                          type="button"
                          className={`px-2 py-1 text-xs rounded ${
                            storageConfig.cacheDurationMs === duration
                              ? 'bg-blue-100 text-blue-700 border border-blue-200'
                              : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                          }`}
                          onClick={() =>
                            storageConfig.setPresetDuration(
                              preset as keyof typeof storageConfig.presetDurations,
                            )
                          }
                        >
                          {preset === 'off'
                            ? 'Off'
                            : storageConfig.formatDuration(duration)}
                        </button>
                      ),
                    )}
                  </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900 truncate ml-2">
                    {status.details.apiConfig.baseUrl}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Tenant:</span>
                  <span className="text-gray-900">
                    {status.details.apiConfig.tenantId}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">User:</span>
                  <span className="text-gray-900">
                    {status.details.apiConfig.userId}
                  </span>
                </div>
              </div>
            </>
