...
function JsonViewMode({ data, level = 0, isExpanded = true, onToggle }: { data: JsonValue; level?: number; isExpanded?: boolean; onToggle?: () => void }) {
                ...
                <div className="space-y-1">
                    {isArray
                        ? (data as JsonArray).map((value, index) => (
                            <div key={`array-${index}-${JSON.stringify(value)}`} className="flex items-start gap-0.5">
                        ...
                        : Object.entries(data as JsonObject).map(([key, value]) => (
                            <div key={`object-${key}-${JSON.stringify(value)}`} className="flex items-start gap-0.5">
                                <div className="flex items-center gap-0.5 text-gray-500">
                                    <Key className="h-3 w-3" />
    ...
    return (
        <div className={cn('space-y-2', className)}>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
                {mode === 'view' ? (
                    <div className="p-4">
                        <JsonViewMode data={parsedData as JsonValue} isExpanded={isExpanded} />
                    </div>
                ) : (
