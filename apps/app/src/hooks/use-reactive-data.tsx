    ...
    useEffect(() => {
        ...
        const initializeReactiveStore = () => {
            try {
                ...
                if (!isMounted) return
                console.error('useReactivePanels: failed to initialize reactive store:', err)
                setError(err instanceof Error ? err.message : 'Failed to initialize reactive store')
                setIsLoading(false)
            }
        }
    ...
    useEffect(() => {
        ...
        const initializeReactiveStore = () => {
            try {
                ...
                if (!isMounted) return
                console.error('useReactivePanel: failed to initialize reactive store:', err)
                setError(err instanceof Error ? err.message : 'Failed to initialize reactive store')
                setIsLoading(false)
            }
        }
    ...
    useEffect(() => {
        ...
        const initializeReactiveStore = () => {
            try {
                ...
                if (!isMounted) return
                console.error('useReactiveView: failed to initialize reactive store:', err)
                setError(err instanceof Error ? err.message : 'Failed to initialize reactive store')
                setIsLoading(false)
            }
        }
...
export function useReactiveColumns(panelId: string) {
    ...
    const refetch = useCallback(() => {
        const storage = reactiveStore?.getStorage()
        if (storage && panelId) {
            storage.getPanel(panelId).then(panel => {
                if (panel) {
                    const allColumns = [
                        ...(panel.patientViewColumns || []),
                        ...(panel.taskViewColumns || [])
                ...
                setError(err instanceof Error ? err.message : 'Failed to refetch columns')
            )
        }
    }, [reactiveStore, panelId])
