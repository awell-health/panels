...
export class ReactivePanelStore {
    ...
    private async initializeStorage(userId?: string, organizationSlug?: string, mode?: StorageMode) {
        try {
            ...
            if (this.storage && 'getReactiveStore' in this.storage) {
                const reactiveAdapter = this.storage as { getReactiveStore(): ReactiveStore }
    ...
    private async loadInitialData() {
        if (!this.storage || !this.reactiveStore) {
            console.log('ReactivePanelStore: storage or reactive store not ready, skipping initial data load')
            return
        }
        ...
        try {
            ...
            if (this.reactiveStore) {
                this.reactiveStore.setError(error instanceof Error ? error.message : 'Failed to load data')
                this.reactiveStore.setLoading(false)
            }
        }
    }
    ...
    async deleteView(panelId: string, viewId: string): Promise<void> {
        ...
        try {
            ...
            if (currentPanel) {
                const updatedPanel = {
                    ...currentPanel,
                    views: (currentPanel.views || []).filter(view => view.id !== viewId)
                }
                this.reactiveStore?.setPanel(updatedPanel)
            }
    ...
    // Column operations
    async updateColumn(panelId: string, columnId: string, updates: Partial<ColumnDefinition>): Promise<void> {
        if (!this.storage) {
            throw new Error('Storage adapter not initialized')
        }
...
// Create context
export const ReactivePanelStoreContext = createContext<ReactivePanelStore | null>(null)

// Provider component
export function ReactivePanelStoreProvider({ children }: { children: React.ReactNode }) {
...
export function useReactivePanelStore() {
    ...
    if (!store) {
        throw new Error('useReactivePanelStore must be used within a ReactivePanelStoreProvider')
    }
