...
async function findPatientByIdentifiers(
  ...
  // Build list of all identifiers to search for
  ...
  // Add profile identifiers if they exist
  if (profile.identifier?.length) {
    for (const id of profile.identifier) {
      const system = safeStringTrim(id.system)
      const value = safeStringTrim(id.value)
  ...
  console.log(
    'Searching for patient with identifiers:',
    JSON.stringify(
      {
        awellPatientId,
        identifiersToSearch,
  ...
  // Search for patient with any of the identifiers
  for (const identifier of identifiersToSearch) {
    try {
      ...
      console.log(
        'Error searching for patient with identifier:',
        JSON.stringify(
          {
            error: error instanceof Error ? error.message : 'Unknown error',
            identifier,
  ...
  console.log(
    'No patient found with any of the identifiers:',
