...
async function fetchHostedPagesLink(pathwayId: string, stakeholderId: string): Promise<string | null> {
  try {
    ...
    console.log('Error fetching hosted pages link:', JSON.stringify({
...
function extractActivityId(task: Task): string | null {
  const awellExtension = task.extension?.find(ext => ext.url === 'https://awellhealth.com/fhir/StructureDefinition/awell-task')?.extension?.find(ext => ext.url === 'activity-id');
  if (awellExtension) {
...
function extractStakeholderId(task: Task): string | null {
  const awellExtension = task.extension?.find(ext => ext.url === 'https://awellhealth.com/fhir/StructureDefinition/awell-task')?.extension?.find(ext => ext.url === 'stakeholder-id');
  if (awellExtension) {
...
function hasConnectorInput(inputs: TaskInput[] | undefined, connectorCode: string): boolean {
  if (!inputs) return false;
  return inputs.some(input => 
    input.type?.coding?.some(coding => 
...
export async function handler(medplum: MedplumClient, event: BotEvent<Task>): Promise<void> {
  ...
  if (!activityId || !pathwayId || !stakeholderId) {
    console.log('No activity ID, pathway ID, or stakeholder ID found in task extensions');
    return;
  }

  if (hasConnectorInput(inputs, 'awell-hosted-pages') && hasConnectorInput(inputs, 'awell-care')) {
    console.log('Connector input already exists for Awell Hosted Pages and Awell Care');
    return;
  }
  ...
  if (!hasConnectorInput(inputs, 'awell-hosted-pages')) {
    ...
    console.log('Adding connector input for Awell Hosted Pages with link:', hostedPagesLink);
    if (hostedPagesLink) {
      const newInput: TaskInput = {
        type: {
          coding: [{
